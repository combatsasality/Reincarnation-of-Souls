...
public class Soul extends Item implements ISoulMaterial {
    ...
    public Soul(int burnTime, SoulType soulType) {
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        if (!world.isClientSide && entity instanceof PlayerEntity) {
            ...
            if (random) {
                if (this.soulType == SoulType.FRIENDLY) player.heal(1);
                else if (this.soulType == SoulType.AGGRESSIVE) player.hurt(new DamageSource("soul").setMagic(), 2.0F);
            }
        }
